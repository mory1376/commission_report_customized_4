...
def execute(filters=None):
	...
	employee = frappe.get_all("Employee", filters={"user_id": user.name}, fields=["name", "employee_number"])

	if employee:
		# Current user has an Employee document
		employee_id = employee[0]["name"]
	else:
		# Current user does not have an Employee document
		employee_id = None
	sales_person = filters.get("sales_person")

	if sales_person:
		# Get Employee ID of the sales person
		employee2 = frappe.get_all("Sales Person",
								   filters={"name": sales_person},
								   fields=["employee"])
		if employee2:
			# Sales person has an Employee document
			employee_id2 = employee2[0]["employee"]
		else:
			# Sales person does not have an Employee document
			employee_id2 = None
	# print(f"\n\n\n current user is : {employee_id}")
	# print(f"\n\n\n\ncurrent filter is : {employee_id2}")
	if employee_id != employee_id2 and "Sales Master Manager" or "Auditor" or "Accounts Manager" not in frappe.get_roles():
		frappe.throw("Only Sales Master Managers are allowed to run this report.")
		return None
	if filters.group_by != "Invoice" and "Sales Master Manager" or "Auditor" or "Accounts Manager" not in frappe.get_roles():
		frappe.throw("Only Sales Master Managers are allowed to run this report.")
		return None
	...
	column_map = frappe._dict(
		{
			...
			"brand_commission_rate": {
				"label": _("Brand Commission Rate"),
				"fieldname": "brand_commission_rate_%",
				"fieldtype": "Percent",
				"width": 100,
			},
			"local_commission_amount": {
				"label": "Approx Commission",
				"fieldname": "local_commission_amount",
				"fieldtype": "Currency",
				"options": "currency",
				"width": 100,
			},
	...
	return frappe._dict(
		{
			...
			"brand_commission_rate": "brand_commission_rate_%",
			"local_commission_amount": "local_commission_amount",
			"project": "project",
		}
	)
	...
	def process(self):
		...
		for row in reversed(self.si_list):
			...
			if _brand_coommission_local:
				...
				row.brand_commission_rate = frappe.db.get_value('Sales Person', self.filters.sales_person,
																'Brandless_commission_local')

			row.allocated_percentage = frappe.get_value("Sales Team",
								 filters={"parent": row.parent, "sales_person": self.filters.sales_person},
								 fieldname="allocated_percentage") or 0.0

			_local_commission_amount = row.brand_commission_rate * row.gross_profit / 100
			row.local_commission_amount = (_local_commission_amount * row.allocated_percentage) / 100

			if grouped_by_invoice:
				if row.indent == 1.0:
					local_commission_amount += row.local_commission_amount
				elif row.indent == 0.0:
					row.local_commission_amount = local_commission_amount
					local_commission_amount = 0
			# add to grouped
