    ...
    def process(self):
        ...
        for row in reversed(self.si_list):
            ...
            if _brand_coommission_local:
                ...
                row.brand_commission_rate = frappe.db.get_value('Sales Person', self.filters.sales_person, 'Brandless_commission_local')

            _local_commission_amount = row.brand_commission_rate * row.gross_profit / 100
            row.local_commission_amount = flt(_local_commission_amount, self.currency_precision)
        ...
        for key in list(self.grouped):
            if self.filters.get("group_by") == "Invoice":
                for i, row in enumerate(self.grouped[key]):
                    if row.indent == 1.0:
                        if (
                                row.parent in self.returned_invoices and row.item_code in self.returned_invoices[
                            row.parent]
                        ):
